...
server = function(input, output){
  ...
  output$ROC <- renderPlot({
    ...
    #             legend = c(paste("Log Model: ", auc),
    #                        paste("Decision Tree: ", auc2),
    #                        paste("Random Forest: ", auc3),
    #                        paste("Naive Bayes: ", auc4),
    #                        paste("K-NN: ", auc5)),
    #          fill = colors)
    # }
  })
  ...
  output$Measures <- renderPlot({
    ...
    #           legend.text = element_text(size = 14)) +
    #     scale_y_continuous(breaks = scales::pretty_breaks(n = 20)) +
    #     geom_text(aes(label=round(value,3), group = variable), position=position_dodge(width=.9), vjust = 2, size=5.5, color = 'white', fontface = 'bold')
    # }
  })
  ...
  output$Importance <- renderPlot({
    if(input$model == 'Logistic Regression') {
      ...
      ggplot(df6, aes(x = Variable, y = Count)) +
        geom_col(aes(fill = Variable)) +
        geom_text(aes(label=round(Count,0)), vjust = 2, size = 6.5, color = 'white', fontface = 'bold') +
        theme(axis.title = element_blank(),
              title = element_text(size = 20, face = 'bold'),
              plot.title = element_text(hjust = 0.5),
              axis.text = element_text(size = 14),
              legend.text = element_text(size = 14)) +
        labs(title = 'Most Important Vaiables Across All Models', fill = 'Predictor')
    }
  })
